...
const listar_usuarios = async (req, res) => {
  ...
  if (search != "") {} else {
    ...
    listar.forEach(element => {
      datos_tabla.data.push(new Array(element.nombre, element.usuario, element.clave, element.tipo, "<a class='_success' onclick='editar(/" + element._id + "/)' title='ver detalle' name='ver_detalle' id='ver_detalle'><i class='fas fa-edit'></i></a> | <a title='ver transporte' class='_success' onclick='eliminar(/" + element._id + "/)' name='ver_detalle' id='ver_detalle'><i class='fas fa-close'></i></a>"));
    });
...
const obtener_por_id_usuarios = async (req, res) => {
  // Devolver al formulario los datos
