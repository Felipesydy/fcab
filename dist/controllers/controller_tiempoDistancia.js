...
const tiempoDistancia = async (req, res) => {
  let event = req.body;
  let resulrepo = {};
  console.log(req);
  ...
  try {
    return new Promise(resolve => {
      ...
      MongoClient.connect(url, function (err, db) {
        ...
        dbo.collection("tiempoDistancia").drop(function (err, delOK) {
          if (err) {
            resolve({ "code": "500", "message": "Drop Collection Error" });
          }
          if (delOK) console.log("Collection deleted");
          dbo.createCollection("tiempoDistancia", function (err, res) {
            if (err) {
            ...
            dbo.collection("tiempoDistancia").insertMany(data, function (err, res) {
              if (err) {
    ...
    console.log({ 'status': 500, 'message': "Error al ejecutar proceso de Distancia tiempo" });
    resolve({ 'status': 500, 'message': "Error al ejecutar proceso de DIstancia Tiempo" });
  }
};

module.exports = { tiempoDistancia };